...
2026-09-20 (current): sections A, B, C are done and proven live (see `docs/build-log.md`, "Router v2", "Policy v2", "Keeper v2"). D1-D8 are implemented in `packages/core`; the SDK live smoke in `docs/build-log.md` verified portfolio, oracle, check, tick simulation, events, codec and unsigned builders. E1-E4 are implemented with mocked model tests. F1-F4 are implemented as local demo backend functions and reference routes: the shared landing/SEP code moved from keeper to core, with a private file transfer store. G is complete: mock oracle admin runs as a separate Next app on port 3100, and the web app reads the oracle contract directly. H1 confirmed the testnet position NFT; `readPositionNft` exposes it. E and F have not had live API smoke tests. The frontend teammate merged a new UI during this session; the new panels need integration verification against the v2 SDK.
