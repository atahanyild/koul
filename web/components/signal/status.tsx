...
export function StatusDot({ on, size = "sm", className }: { on: boolean; size?: "sm" | "md"; className?: string }) {
  const cls = cn("inline-block shrink-0 rounded-full", size === "sm" ? "size-2.5" : "size-3.5", on ? "bg-accent-text" : "border-2 border-dim", className);
...
export function Label({ tone = "muted", className, children, ...rest }: React.HTMLAttributes<HTMLSpanElement> & { tone?: "muted" | "dim" | "lime" | "text" | "danger" | "onLime" }) {
  const t = { muted: "text-muted", dim: "text-dim", lime: "text-accent-text", text: "text-text", danger: "text-danger", onLime: "text-on-lime" }[tone];
  return (
