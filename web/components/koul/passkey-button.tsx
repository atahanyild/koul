...
export function PasskeyHint({ phase, error, onRetry }: { phase: ActionPhase; error?: { userMessage?: string; message: string } | null; onRetry?: () => void }) {
  if (phase === "prompt") return <p className="text-xs text-muted-foreground" role="status">Your device is asking for Face ID or Touch ID. Nothing is sent until you approve.</p>;
